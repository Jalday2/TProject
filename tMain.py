...
for x in range(0, countObjM):
	#opens and imports object and input files
	...
	#this for loop takes commas out of ouptut files then converts those files into numpy arrays and exports them to excel
	for j in range(2):
		...
		if (j == 0):
			MJDOrb, SMA, ECC, INC, RAAN, AOP, MA = np.loadtxt(TextOut[j], skiprows = 3, unpack = True)
			os.remove("./out/orbelsNoC.dat")
			#print("\nMJD\n",MJDOrb,"\nSMA\n",SMA,"\nECC\n",ECC,"\nINC\n",INC,"\nRAAN\n",RAAN,"\nAOP\n",AOP,"\nMA\n",MA,"\nend orbitals")
			#export excel
			outLoc = "./out/ExcellFiles/Orbitals" + str(x+1) + ".xlsx"
			df = pd.DataFrame({"MJD" : MJDOrb, "SMA": SMA, "ECC" : ECC, "INC" : INC,"RAAN" : RAAN, "AOP" : AOP, "MA" : MA})
			df.to_excel(excel_writer = outLoc)

		else:
			MJDCart, X, Y, Z, VX, VY, VZ = np.loadtxt(TextOut[j], skiprows = 3, unpack = True)
			os.remove("./out/cartNoC.dat")
			#print("\nMJD\n",MJDCart,"\nX\n",X,"\nY\n",Y,"\nZ\n",Z,"\nVX\n",VX,"\nVY\n",VY,"\nVZ\n",VZ)	
			#export excel
			outLoc = "./out/ExcellFiles/Cartesian" + str(x+1) + ".xlsx"
